...
def build_v6_screener(tickers_list, offline=False, slider_6m=0.30, slider_1y=0.40):
    ...
    for ticker in processing_list:
        ...
        if pd.notna(ret_6m) and ret_6m >= slider_6m and pd.notna(ret_1y) and ret_1y >= slider_1y:
            survivors.append({
                "ticker": ticker, "close": close, "avg_vol_lakhs": avg_vol_lakhs,
                "ret_1m": ret_1m, "ret_6m": ret_6m, "ret_1y": ret_1y, "ret_3y": ret_3y,
                "structural": structural, "structure_reason": structure_reason
            })
    ...
    for i, stock in enumerate(survivors):
                ...
                if q_fin is not None and not q_fin.empty and 'Net Income' in q_fin.index:
                    ...
                    if len(ni) >= 3:
                        ...
                        for j in range(min(3, len(ni)-1)):
                            if ni.iloc[j+1] != 0: growth_rates.append((ni.iloc[j] / ni.iloc[j+1]) - 1)
...
with tab1:
    ...
    if not master_df.empty:
        top_df = master_df[master_df['Rating'].str.contains('👑|🟢|🟡')].copy() if not offline_mode else master_df.copy()
...
with tab3:
    ...
    if not master_df.empty:
        ...
        with colA:
            stock_list = master_df.sort_values('Score', ascending=False)['Ticker'].tolist()
            selected = st.selectbox("Select Stock:", stock_list)
            row = master_df[master_df['Ticker'] == selected].iloc[0]
        ...
        with colB:
            if full_ticker in hist_data['Close'].columns:
